...
def update_readme_version():
    ...
    docs_readme_path = pathlib.Path(__file__).parent.parent / os.path.join('docs', 'README.md')
...
def update_version_in_file(file_path, version):
    ...
    updated_content = re.sub(r'Version[0-9]+\.[0-9]+\.[0-9]+', f"Version {version}", content)
